...
/*   Updated: 2024/06/28 19:04:19 by jolopez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cube.h"

/*  Function ft_set_pixels() iterates over the block size to set each pixel
...
/*	This function set all the pixels in a block of minimap to the right color
	according to the player, a wall, the floor, or nothing on the map.*/
static void	ft_minimap_block(t_minimap *minimap, int x, int y)
{
	if (minimap->map[y][x] == 'P')
		ft_set_pixels(minimap, x * minimap->block_size,
			y * minimap->block_size, MINIMAP_PLAYER_COLOR);
	else if (minimap->map[y][x] == '1')
		ft_set_pixels(minimap, x * minimap->block_size,
			y * minimap->block_size, MINIMAP_WALL_COLOR);
	else if (minimap->map[y][x] == '0')
		ft_set_pixels(minimap, x * minimap->block_size,
			y * minimap->block_size, MINIMAP_FLOOR_COLOR);
	else if (minimap->map[y][x] == ' ')
		ft_set_pixels(minimap, x * minimap->block_size,
			y * minimap->block_size, MINIMAP_SPACE_COLOR);
}

/*	This function calculates the size of the border based on the minimap pixel
	size and block size iterating over each pixel within the calculated
...
static void	ft_minimap_border(t_minimap *minimap, int color)
{
	...
	size = MINIMAP_PXL_SIZE + minimap->block_size;
	y = 0;
	while (y < size)
	{
		x = 0;
...
/* This function renders the minimap based on the map data iterating through
	...
	ft_minimap_block() to draw the corresponding block on the minimap at the
...
/*	Function ft_render_minimap_img():
	 -> Calculates the size of the minimap image based on the predefined 
	 	minimap pixel size and the block size.
...
void	ft_render_minimap_img(t_data *data, t_minimap *minimap)
{
	...
	img_size = MINIMAP_PXL_SIZE + minimap->block_size;
	ft_mlx_img(data, &data->minimap_img, img_size, img_size);
	ft_draw_minimap(minimap);
	mlx_put_image_to_window(data->mlx, data->window,
		data->minimap_img.img, minimap->block_size,
		data->window_height - (MINIMAP_PXL_SIZE + (minimap->block_size * 2)));
